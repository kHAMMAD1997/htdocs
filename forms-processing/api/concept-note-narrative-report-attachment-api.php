...
$uploadDir = __DIR__ . '/upload/concept-note-narrative-report/';
if (!is_dir($uploadDir)) {
...
function handlePost()
{
    ...
    if (!isset($_POST['report_id']) || empty($_FILES['files']['name'][0])) {
        http_response_code(400);
        echo json_encode(["error" => "report_id and files are required"]);
        exit;
    }
    ...
    for ($i = 0; $i < $fileCount; $i++) {
        ...
        $newFileName = "{$report_id}-" . ($i + 1) . "-{$originalFileName}";
        $filePath = $uploadDir . $newFileName;

        if (move_uploaded_file($_FILES['files']['tmp_name'][$i], $filePath)) {
            $relativePath = "/upload/concept-note-narrative-report/" . $newFileName;
            $stmt = $conn->prepare("INSERT INTO concept_note_narrative_report_attachment (report_id, file_path) VALUES (:report_id, :file_path)");
            $stmt->execute([
                ':report_id' => $report_id,
...
// ✅ GET: Retrieve all files for a specific report_id
function handleGet()
{
    global $conn;
    ...
    $stmt = $conn->prepare("SELECT id, file_path FROM concept_note_narrative_report_attachment WHERE report_id = :report_id");
...
function handlePatch()
{
    ...
    $stmt = $conn->prepare("SELECT file_path FROM concept_note_narrative_report_attachment WHERE id = :id");
    $stmt->execute([':id' => $id]);
    ...
    $newRelativePath = "/upload/concept-note-narrative-report/" . $newFileName;

    if (rename($oldFullPath, $newFullPath)) {
        $stmt = $conn->prepare("UPDATE concept_note_narrative_report_attachment SET file_path = :file_path WHERE id = :id");
        $stmt->execute([
...
function handleDelete()
{
    ...
    $stmt = $conn->prepare("DELETE FROM concept_note_narrative_report_attachment WHERE id = :id");
    $stmt->execute([':id' => $id]);
