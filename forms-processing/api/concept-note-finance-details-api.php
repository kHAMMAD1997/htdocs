...
switch ($method) {
    case 'POST':
        ...
        if (isset($data['form_id'], $data['breakdown'])) {
            try {
                $stmt = $conn->prepare("INSERT INTO concept_note_finance_details (form_id, breakdown) VALUES (:form_id, :breakdown)");
    ...
    case 'GET':
        ...
        if ($id) {
            $stmt = $conn->prepare("SELECT * FROM concept_note_finance_details WHERE id = :id");
            $stmt->bindValue(":id", $id, PDO::PARAM_INT);
        } elseif ($form_id) {
            $stmt = $conn->prepare("SELECT * FROM concept_note_finance_details WHERE form_id = :form_id");
    ...
    case 'PATCH':
        ...
        if (isset($data['id'], $data['breakdown'])) {
            try {
                $stmt = $conn->prepare("UPDATE concept_note_finance_details SET breakdown = :breakdown WHERE id = :id");
                $stmt->bindValue(":breakdown", $data['breakdown'], PDO::PARAM_STR);
    ...
    case 'DELETE':
        ...
        if (isset($data['id'])) {
            try {
                $stmt = $conn->prepare("DELETE FROM concept_note_finance_details WHERE id = :id");
                $stmt->bindValue(":id", $data['id'], PDO::PARAM_INT);
