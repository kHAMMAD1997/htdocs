...
$uploadDir = __DIR__ . '/upload/short-concept-note/';
if (!is_dir($uploadDir)) {
...
function handlePost()
{
    ...
    for ($i = 0; $i < $fileCount; $i++) {
        ...
        if (move_uploaded_file($_FILES['files']['tmp_name'][$i], $filePath)) {
            $relativePath = "/upload/short-concept-note/" . $newFileName;
            $stmt = $conn->prepare("INSERT INTO short_concept_note_finance_attachment (concept_note_id, file_path) VALUES (:concept_note_id, :file_path)");
            $stmt->execute([
...
function handleGet()
{
    ...
    $stmt = $conn->prepare("SELECT id, file_path FROM short_concept_note_finance_attachment WHERE concept_note_id = :concept_note_id");
...
function handlePatch()
{
    ...
    $stmt = $conn->prepare("SELECT file_path FROM short_concept_note_finance_attachment WHERE id = :id");
    $stmt->execute([':id' => $id]);
    ...
    $newRelativePath = "/upload/short-concept-note/" . $newFileName;

    if (rename($oldFullPath, $newFullPath)) {
        $stmt = $conn->prepare("UPDATE short_concept_note_finance_attachment SET file_path = :file_path WHERE id = :id");
        $stmt->execute([
...
function handleDelete()
{
    ...
    $stmt = $conn->prepare("DELETE FROM short_concept_note_finance_attachment WHERE id = :id");
    $stmt->execute([':id' => $id]);
