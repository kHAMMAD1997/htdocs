...
function handleGet($conn) {
    $sql = "SELECT * FROM concept_note_narrative_report_metadata WHERE 1=1";
    $params = [];
...
function handlePost($conn) {
    ...
    $sql = "INSERT INTO concept_note_narrative_report_metadata (report_id, approve, revise, finance, declined) 
            VALUES (:report_id, :approve, :revise, :finance, :declined)";
    $stmt = $conn->prepare($sql);

    $stmt->bindValue(':report_id', $input['report_id'], PDO::PARAM_INT);
...
function handlePatch($conn) {
    ...
    $sql = "UPDATE concept_note_narrative_report_metadata SET " . implode(", ", $updates) . " WHERE metadata_id = :metadata_id";
    $stmt = $conn->prepare($sql);
...
function handleDelete($conn) {
    ...
    $sql = "DELETE FROM concept_note_narrative_report_metadata WHERE metadata_id = :metadata_id";
    $stmt = $conn->prepare($sql);
