...
function handleGet($conn) {
    $sql = "SELECT * FROM grant_application_metadata WHERE 1=1";
    $params = [];
    ...
    if (!empty($_GET['grant_application_id'])) {
        $sql .= " AND grant_application_id = :grant_application_id";
        $params[':grant_application_id'] = $_GET['grant_application_id'];
    }
...
function handlePost($conn) {
    ...
    if (empty($input['grant_application_id'])) {
        echo json_encode(["error" => "grant_application_id is required"]);
        return;
    }

    $sql = "INSERT INTO grant_application_metadata (grant_application_id, approve, revise, finance, declined) 
            VALUES (:grant_application_id, :approve, :revise, :finance, :declined)";
    $stmt = $conn->prepare($sql);

    $stmt->bindValue(':grant_application_id', $input['grant_application_id'], PDO::PARAM_INT);
...
function handlePatch($conn) {
    ...
    $sql = "UPDATE grant_application_metadata SET " . implode(", ", $updates) . " WHERE metadata_id = :metadata_id";
    $stmt = $conn->prepare($sql);
