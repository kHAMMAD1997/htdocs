...
if ($requestMethod== "POST") {
    // Handle creating a new decline comment
    $data = json_decode(file_get_contents("php://input"), true);

    $query = "INSERT INTO concept_narrative_report_decline_comment (report_id, comment, submission_date, author_id)
              VALUES (:report_id, :comment, :submission_date, :author_id)";

    $stmt = $conn->prepare($query);
    $stmt->bindParam(':report_id', $data['report_id'], PDO::PARAM_INT);
    ...
    // Handle updating an existing decline comment
    $data = json_decode(file_get_contents("php://input"), true);
    ...
    $query = "UPDATE concept_narrative_report_decline_comment 
              SET report_id = :report_id, comment = :comment, submission_date = :submission_date, author_id = :author_id 
    ...
    $stmt = $conn->prepare("DELETE FROM concept_narrative_report_decline_comment WHERE comment_id = :comment_id");
    ...
    $query = "SELECT * FROM concept_narrative_report_decline_comment WHERE 1=1";
    $params = [];

    $allowedParams = ["comment_id", "report_id", "comment", "submission_date", "author_id"];
