...
if ($requestMethod === "POST") {
    ...
    $query = "INSERT INTO grant_application_delete_comment (grant_application_id, comment, submission_date, author_id)
              VALUES (:grant_application_id, :comment, :submission_date, :author_id)";

    $stmt = $conn->prepare($query);
    $stmt->bindParam(':grant_application_id', $data['grant_application_id'], PDO::PARAM_INT);
    ...
    $query = "UPDATE grant_application_delete_comment 
              SET grant_application_id = :grant_application_id, comment = :comment, submission_date = :submission_date, author_id = :author_id 
    ...
    $stmt = $conn->prepare("DELETE FROM grant_application_delete_comment WHERE comment_id = :comment_id");
    ...
    $query = "SELECT * FROM grant_application_delete_comment WHERE 1=1";
    $params = [];

    $allowedParams = ["comment_id", "grant_application_id", "comment", "submission_date", "author_id"];
